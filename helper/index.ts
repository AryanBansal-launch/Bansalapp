...
export const getSkillsResCached = async (q: string,entryUrl: string): Promise<Page> => {
  // const t1 = new Date();
  const response = await fetch(
    `https://cdn.contentstack.io/v3/content_types/common_page/entries/bltbd861b1a14f9288e`,
    {
...
export const getSkillsResCloudflare = async (q: string,entryUrl: string):Promise<any> => {
  ...
  const t2= new Date();
  console.log("Time from Cloudflare:", t2.getTime() - t1.getTime(),"x-request-id:", response.headers.get("x-request-id"),"x-cache:", response.headers.get("x-cache"),"query:",q);
