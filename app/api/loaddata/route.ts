...
export async function GET(req: NextRequest) {
  try {
    ...
    let comments: Comment[] = await commentsResponse.json();
    ...
    comments = comments.map((comment) => {
      ...
      const consonants =
        comment.body.match(/[bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ]/g)
          ?.length || 0;
    ...
    comments.sort((a, b) => b.vowels - a.vowels); // Sort comments by number of vowels
    ...
    // Fetch projects
    const projectResponse: Project[] = await getProjectsRes("/projects");
    const projects: string[] = projectResponse.map(
